...
namespace Lib_RscViewers
{
    ...
    public partial class RscViewer_PhotoFolderV10 : PhoneApplicationPage
    {
        ...
        public RscViewer_PhotoFolderV10()
        {
            InitializeComponent();
			...
			m_AppFrame = new RscAppFrame("Ressive.Hu", "Photo Gallery 1.0", "Images/IcoSm001_Gallery.jpg"
			...
			string sPath = RscKnownFolders.GetMediaPath("DCIM");
		...
		private void m_tmrBrowse_Tick(object sender, System.EventArgs e)
		{
			...
			appInput.SetFlag( 0, "image folder path" );
				...
				for( int i = 0; i < m_iCX; i++ )
				{
					if( gal.m_a[ i ].Name != "blank" )
					{
						...
						else
						{
							...
							ib.Image.Source = LoadImage(gal.m_a[ i ].Path, iDim, iDim);
						}
				...
				for( int iX = iXstart; iX <= iXend; iX++ )
				{
					if( bChecked )
					{
						if( !m_a[ iY ].m_a[ iX ].Checked )
						{
							...
							if( m_a[ iY ].m_a[ iX ].Path.Length > 0 )
							{
		...
		private void BuildListStep()
		{
			...
			m_Root.Parse( "Image", true, (!m_bOrderAscending) );
				...
				for( int iX = 0; iX < m_iCX; iX++ )
				{
					...
					else
					{
						...
						if( !galimg.FsItem.Folder )
						{
							...
							if( store.FileExists( sTn + ".jpg" ) )
							{
								//m_AppFrame.TRACE = sTn;
								
								galimg.ThumbPath = sTn + ".jpg";
							}
							else if( store.FileExists( sTn + galimg.FsItem.FileExt ) )
							{
								//m_AppFrame.TRACE = sTn;
								
								galimg.ThumbPath = sTn + galimg.FsItem.FileExt;
							}
						}
					}
				}
