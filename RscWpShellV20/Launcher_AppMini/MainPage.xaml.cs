...
using Launcher_AppMini.Resources;
...
namespace Launcher_AppMini
{
	
    public partial class MainPage : PhoneApplicationPage
    {
		
		const string csTitle = "#Launcher 80 Mini";
		
		bool m_bNO_IE = false;
		bool m_bNO_Explorer = false;
		...
		private void AddApps( )
		{
			...
			if( !m_bNO_Explorer )
			{
				m_asApps.Add( new GalleryItem(
					"/Lib_RscViewers;component/RscViewer_FsV12",
					"Images/Ico001_Explorer4.jpg",
					"Explorer",
					"" ) );
			}
			
			m_asApps.Add( new GalleryItem(
				"/RscFtpClients;component/MainPage",
				"Images/Ico001_FtpTest.jpg",
				"FTP Cloud",
				"" ) );
			
			m_asApps.Add( new GalleryItem(
				"/Lib_RscSettings;component/MainPage",
				"Images/Ico001_Settings.jpg",
				"Settings",
				"" ) );
			
			m_asApps.Add( new GalleryItem(
				"/Lib_RscViewers;component/RscViewer_TextV10",
				"Images/Ico001_EventViewer.jpg",
				"Event Viewer",
				"?folder=A:\\System\\Events" ) );
			
			m_asApps.Add( new GalleryItem(
				"/RscLearnXamlV10;component/ClockSaver",
				"Images/Ico001_ScreenSaver.jpg",
				"Screen Saver",
				"" ) );
			...
			m_asApps.Add( new GalleryItem(
				"/RscLearnXamlV10;component/ColorMatchGame",
				"Images/Ico001_ColorMatch.jpg",
				"Color Match Game",
				"" ) );
		...
		class GalleryItem
		{
			...
			public GalleryItem(string sPage, string sImage, string sTitle, string sArgs)
			{
				Page = sPage;
				Image = sImage;
				Title = sTitle;
        ...
        public MainPage()
        {
			...
			m_AppFrame = new RscAppFrame("Ressive.Hu", csTitle, "Images/Ico001_LauncherMini.jpg"
		...
		protected override void OnNavigatedTo(System.Windows.Navigation.NavigationEventArgs e)
		{
			...
			if( !m_bAppStarted ) //FIX: Returning from ThumbStuning, Dlgs issues...
			{
				...
				if( parameters.ContainsKey( "NoIe" ) )
				{
					string sOp = parameters["NoIe"];
					if( sOp == "True" )
						m_bNO_IE = true;
				}
				if( parameters.ContainsKey( "NoExplorer" ) )
				{
					string sOp = parameters["NoExplorer"];
					if( sOp == "True" )
						m_bNO_Explorer = true;
				}
        ...
        private void BuildList()
        {
			...
			{				
				for( int iX = 0; iX < m_iCX; iX++ )
				{
					...
					GalleryItem galimg = new GalleryItem( "", "", "N/A", "" );
					gal.m_a.Add( galimg );
					...
					{
						galimg.Page = m_asApps[ iFle ].Page;
						galimg.Image = m_asApps[ iFle ].Image;
						galimg.Title = m_asApps[ iFle ].Title;
						galimg.Args = m_asApps[ iFle ].Args;
					}
				}
			...
			for( int i = 0; i < m_iCX; i++ )
			{
				if( gal.m_a[ i ].Page != "blank" )
				{
					...
					try
					{
						...
						/*
						BitmapImage bmp = new BitmapImage(new Uri("/RscLib" + ";component/"
							+ sImg, UriKind.Relative));
						ib.Image.Source = bmp;
						*/
						ib.Image.Source = m_AppFrame.Theme.GetImage(gal.m_a[ i ].Image);
		...
		private void m_Image_Click(object sender, System.Windows.RoutedEventArgs e)
		{
			...
			string sUri = m_a[ iYcurr ].m_a[ iXcurr ].Page + ".xaml" + m_a[ iYcurr ].m_a[ iXcurr ].Args;
