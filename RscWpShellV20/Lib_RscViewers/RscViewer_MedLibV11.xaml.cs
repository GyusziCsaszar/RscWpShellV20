...
namespace Lib_RscViewers
{
    ...
    public partial class RscViewer_MedLibV11 : PhoneApplicationPage
    {
        ...
        public RscViewer_MedLibV11()
        {
            InitializeComponent();
  			
			m_AppFrame = new RscAppFrame("Ressive.Hu", "Media Library 1.1", "Images/Ico001_Ressive.jpg"
			...
			m_txtTitle.Text = "TIP: Press Next to refresh.";
			
			m_btnThumbnail = new RscIconButton(ImagePanel, Grid.ColumnProperty, 0, 48, 80, Rsc.Collapsed);
			...
			m_btnImage = new RscIconButton(ImagePanel, Grid.ColumnProperty, 1, 96, 160, Rsc.Collapsed);
			...
			m_btnIsoImage = new RscIconButton(ImagePanel, Grid.ColumnProperty, 3, 96, 160, Rsc.Collapsed);
			//m_btnIsoImage.Image.Source = m_AppFrame.DefaultedImageList.GetImage("Images/Btn001_Continue.jpg");
		...
		{
			m_txtTitle.Text = "TIP: Tap item to show image.";
		...
		private string GetImagePath( string sIndiciesToItem, bool bThumbnail, out Picture pic, bool bForceCreateFolderPath = false )
		{
			...
			for( int i = 0; i < iCnt; i++ )
			{
				if( i == iCnt - 1 )
				{
					...
					{
						if( !bForceCreateFolderPath ) return "";
